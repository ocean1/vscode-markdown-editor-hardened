...
// vditor 3.10+ removed the i18n module export — strings now hang off
// `window.VditorI18n` (populated by vditor at init based on its `lang`
// option). See upstream PR #142 for the API-change context.
declare global {
  interface Window {
...
export function fixTableIr() {
  ...
  function insertTablePanel() {
    ...
    if (!tablePanel) {
      ...
      // vditor 3.10+ exposes its i18n table at window.VditorI18n; fall back
      // to `{}` so the `|| 'Fallback'` clauses below kick in if vditor
      // hasn't populated it yet (e.g., during early render).
      const i18n: Record<string, string> = (window as any).VditorI18n || {}
